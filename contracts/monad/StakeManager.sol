...
contract StakeManager is Initializable, Manager, UUPSUpgradeable {
    ...
    mapping(address => EnumerableSet.UintSet) _validatorsOf;
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _validators.length; ++i) {
            _validatorsOf[_poolAddress].add(_validators[i]);
        }
    }
    ...
    function getValidatorsOf(address _poolAddress) public view returns (uint64[] memory validators) {
        uint256[] memory vals256 = _validatorsOf[_poolAddress].values();
        validators = new uint64[](vals256.length);

        for (uint256 i = 0; i < vals256.length; i++) {
            validators[i] = uint64(vals256[i]);
        }
    }
    // ------------ settings ------------
    ...
    function rmStakePool(address _poolAddress) external onlyOwner {
        ...
        uint64[] memory validators = getValidatorsOf(_poolAddress);
        for (uint256 j = 0; j < validators.length; ++j) {
            if (IMonadStakePool(_poolAddress).getDelegated(validators[j]) != 0) revert DelegateNotEmpty();

            _validatorsOf[_poolAddress].remove(validators[j]);
        }

        if (!bondedPools.remove(_poolAddress)) revert PoolNotExist(_poolAddress);
    }

    function addValidator(address _poolAddress, uint64 _validator) external onlyOwner {
        if (_validatorsOf[_poolAddress].length() >= MAX_VALIDATORS_LEN) revert ValidatorsLenExceedLimit();
        if (!_validatorsOf[_poolAddress].add(_validator)) revert ValidatorDuplicated();
    }

    function rmValidator(address _poolAddress, uint64 _validator) external onlyOwner {
        if (IMonadStakePool(_poolAddress).getDelegated(_validator) != 0) revert DelegateNotEmpty();
        if (!_validatorsOf[_poolAddress].remove(_validator)) revert ValidatorNotExist();
    }
    ...
    function withdrawWithPool(address _poolAddress) public {
        ...
        IMonadStakePool(_poolAddress).withdrawForStaker(msg.sender, totalWithdrawAmount);
    ...
    function newEra() external {
        ...
        for (uint256 i = 0; i < poolList.length; ++i) {
            ...
            IMonadStakePool stakePool = IMonadStakePool(poolAddress);
            uint64[] memory validators = getValidatorsOf(poolAddress);

            // claim undelegated
            stakePool.withdrawMulti(validators);
            ...
            // compound
            stakePool.compoundMulti(validators);
