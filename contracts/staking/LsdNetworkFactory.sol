...
contract LsdNetworkFactory is Initializable, UUPSUpgradeable, ILsdNetworkFactory {
    ...
    function createLsdNetwork(string memory _lsdTokenName, string memory _lsdTokenSymbol, uint256 _stakingPoolId)
        external
        override
    {
        _createLsdNetwork(_lsdTokenName, _lsdTokenSymbol, _stakingPoolId, msg.sender);
    }
    ...
    function createLsdNetworkWithTimelock(
        ...
        _createLsdNetwork(_lsdTokenName, _lsdTokenSymbol, _stakingPoolId, networkAdmin);
    }
    ...
    function _createLsdNetwork(
        ...
        (bool success, bytes memory data) = contracts._stakePool.call(
            abi.encodeWithSelector(
                StakePool.initialize.selector, contracts._stakeManager, stakingAddress, _networkAdmin, _stakingPoolId
            )
        );
        if (!success) {
            revert FailedToCall();
