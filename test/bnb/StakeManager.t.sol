...
contract MockBnbStaking is IStaking {
    ...
    function delegate(address validator, uint256 amount) external virtual payable {
        delegated[msg.sender][validator] = amount;
        totalDelegatedOf[msg.sender] += amount;
    }

    function undelegate(address validator, uint256 amount) external virtual payable {}

    function redelegate(address validatorSrc, address validatorDst, uint256 amount) external virtual payable {}

    function claimReward() external virtual pure returns (uint256) {
        return 0;
    }

    function claimUndelegated() external virtual pure returns (uint256) {
        return 0;
    }
    ...
    function getTotalDelegated(address delegator) external virtual view returns (uint256) {
        return totalDelegatedOf[delegator];
    }

    function getDistributedReward(address delegator) external virtual pure returns (uint256) {
        delegator;
        return 0;
    }
    ...
    function getUndelegated(address delegator) external virtual pure returns (uint256) {
        delegator;
        return 0;
    }

    function getPendingUndelegateTime(address delegator, address validator) external virtual pure returns (uint256) {
        delegator;
        validator;
        return 0;
    }

    function getRelayerFee() external virtual pure returns (uint256) {
        return 16e14;
    }

    function getMinDelegation() external virtual pure returns (uint256) {
        return 1e18;
    }

    function getRequestInFly(address delegator) external virtual pure returns (uint256[3] memory) {
        delegator;
